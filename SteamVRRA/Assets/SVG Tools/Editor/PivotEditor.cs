...
namespace svgtools
{
	public class PivotEditor: EditorWindow 
	{
		...
		{  
			...
			element[0].Set(AssetInspector.instance.pivot.vector2Value.x * width + min.x - 8.0f, (1.0f - AssetInspector.instance.pivot.vector2Value.y) * height + max.y + 8.0f, 16.0f, 16.0f);  
			...
			if(Event.current.alt && 1 == Event.current.button)
                EditorGUIUtility.AddCursorRect(viewRect, MouseCursor.Zoom);   
			else if(Event.current.alt || 1 == Event.current.button)
                EditorGUIUtility.AddCursorRect(viewRect, MouseCursor.Pan);
			
			if(EventType.ScrollWheel == Event.current.type || (EventType.MouseDrag == Event.current.type && Event.current.alt && 1 == Event.current.button)) 
			{	
				zoom = zoom * (1.0f + Event.current.delta.y * 0.03f); 
				zoom = Mathf.Clamp(zoom, (Mathf.Min(viewRect.width/size.x, viewRect.height/size.y) * 0.9f), 50.0f);
				Event.current.Use();
			}  
			else if(EventType.MouseDrag == Event.current.type && ((Event.current.alt && 0 == Event.current.button) || 1 == Event.current.button)) 
			{	
			...
			else if(EventType.MouseDown == Event.current.type && 0 == Event.current.button)
			{
				for(int i=0; i<13; ++i) 
				{
					if(element[i].Contains(Event.current.mousePosition))
					{ 
						active = i;	
  						break;
					}
			...
			else if(EventType.MouseUp == Event.current.type && 0 == Event.current.button && -1 != active) 
			{  
				active = -1; 
				EditorGUIUtility.SetWantsMouseJumping(0);
				Event.current.Use();
			}
			else if(EventType.MouseDrag == Event.current.type && -1 != active) 
			{	
				if(0 == active) 
				{
  					pivot.Set(min.x + AssetInspector.instance.pivot.vector2Value.x*width, min.y - AssetInspector.instance.pivot.vector2Value.y*height);	
					pivot += Event.current.delta;  
			...
			if((EventType.MouseUp == Event.current.type || EventType.MouseDown == Event.current.type) && (Event.current.alt || 1 == Event.current.button) || ((EventType.KeyUp == Event.current.type || EventType.KeyDown == Event.current.type) && (KeyCode.LeftAlt == Event.current.keyCode || KeyCode.RightAlt == Event.current.keyCode)))
                base.Repaint(); 
			...
			if(9 != AssetInspector.instance.pivotIndex.intValue)  
			{
				pivot.x = 0.5f * (AssetInspector.instance.pivotIndex.intValue % 3); 
				pivot.y = 0.5f * (AssetInspector.instance.pivotIndex.intValue / 3);	 
				AssetInspector.instance.pivot.vector2Value = pivot;
			}   
			
			GUI.enabled = (9 == AssetInspector.instance.pivotIndex.intValue);			
			EditorGUI.PropertyField(new UnityEngine.Rect(5.0f, 58.0f, 420.0f, 32.0f), AssetInspector.instance.pivot, AssetInspector.customPivotPropertyField);
			GUI.enabled = true;	
			
			EditorGUI.LabelField(new UnityEngine.Rect(5.0f, 80.0f, 70.0f, 16.0f), "Scale9Grid");  
			EditorGUIUtility.labelWidth = 12.0f;
									
			s9g.x = EditorGUI.FloatField(new UnityEngine.Rect(155.0f, 80.0f, 88.0f, 16.0f), "L", s9g.x/width*size.x); 
			s9g.y = EditorGUI.FloatField(new UnityEngine.Rect(245.0f, 80.0f, 88.0f, 16.0f), "T", s9g.y/height*size.y); 
			s9g.z = EditorGUI.FloatField(new UnityEngine.Rect(155.0f, 96.0f, 88.0f, 16.0f), "R", s9g.z/width*size.x);	
			s9g.w = EditorGUI.FloatField(new UnityEngine.Rect(245.0f, 96.0f, 88.0f, 16.0f), "B", s9g.w/height*size.y); 
